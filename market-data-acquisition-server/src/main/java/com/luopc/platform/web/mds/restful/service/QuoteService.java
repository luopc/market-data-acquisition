...
public class QuoteService {
    ...
    public List<QuoteVO> queryBankQuote(QuoteQueryForm queryForm) {
        ...
        if (StringUtils.isNoneBlank(queryForm.getTenor())) {
            ...
            if (CollectionUtils.isNotEmpty(bankQuotesForCcy)) {
                return bankQuotesForCcy.stream().map(MarketDataConvertor::convertToQuoteVO).collect(Collectors.toList());
            } else {
    ...
    private List<QuoteVO> getQuoteListResponseByTenor(List<QuoteByTenor> bankQuotesForTenor, Currency baseCcy, Currency quoteCcy) {
