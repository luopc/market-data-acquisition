...
if [ "$BUILD_TYPE" = "release" ]; then
  ...
  echo -e "${GREEN}deploy -p mdas -v $VERSION -h data${NC}"
else
  echo -e "${RED}Error: Invalid build type '$BUILD_TYPE'${NC}"
  usage
fi

echo -e "${NC}java -jar -Dspring.profiles.active=dev market-data-acquisition-server/target/market-data-acquisition-server.jar${NC}"
